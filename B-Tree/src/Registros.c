...
void buscarRegistros(char *binName, int N) {
  ...
  for (int i = 0; i < N; i++) { // Executa N vezes
    Busca filtro =
        inputFiltro(); // Cria um filtro para a busca (Simula o WHERE)

    fseek(fileBin, TAM_CABECALHO,
          SEEK_SET); // Coloca a referência para o arquivo no primeiro registro
                     // de dados
    ...
    while (fread(&reg.removido, sizeof(char), 1, fileBin) == 1) {
      ...
      if (coincide) {
        // Imprime usando a mesma lógica da listarRegistros, porem se tiver -1
        // imprime NULO
        // 1. codEstacao
        if (reg.codEstacao != -1)
          printf("%d ", reg.codEstacao);
...
void removerRegistros(char *binName, int N) {
  ...
  for (int i = 0; i < N; i++) { // Executa N vezes
    ...
    fseek(fileBin, TAM_CABECALHO,
          SEEK_SET); // Coloca o fseek no primeiro registro de dados
